...
按照学习的视频大概的做了一个小项目，没有100%按视频中的来，有一些自己乱写的。 学习技术最主要的还是要自己动手，多写代码。 

这个小程序主要是实现了一个智慧社区的功能。也就实现了一些简单的功能，没有太复杂的逻辑。本质上是要学习小程序的样式和布局。  
...
## 欢迎页面
  ...
  用一个大图片盖住，3S之后跳转到首页。要注意跳转的页面，要在 tabBar 中配置好 tabBar ，否则会报错。

  这个大图片可以从后端接口请求过来，如果没有再用默认的本地图。
...
首页是用户进入小程序之后，这是一个比较重要的页面，一般用户从欢迎页之后，看到的就是首页，也在内容最多的一个页面。主要分为以下几个模块：
 - 轮播图：动态
 - 公共栏：动态
 - 主要菜单
 - 底部图片
...
这是通过 js 从后端接口加载轮播图数据之后，再渲染到页面上的。
...
读出配置图本的方式，点击之后通过navigateTo 跳转到其他的页面。 wxml 代码如下：
...
2、最好把请求后端的接口，也放到同一个模块中, 代码的模块化更好一些。
...
- 4 采集详情页面
  - 打开摄像头拍照：
   ...
   微信摄像头的文档： https://developers.weixin.qq.com/miniprogram/dev/component/camera.html
...
小程序打包是，代码包最大不能超2M，所以，所有图片最好都放到服务器上，不要放到本地。

```sh 
message：自动真机调试 Error: 系统错误，错误码：80051,source size 53394KB exceed max limit 2MB [20251017 18:25:25][wxcef05fdda3ee67c3]
appid: wxcef05fdda3ee67c3
openid: o6zAJs1xMzE7GzAk8Nkcj3VTLx7Y
ideVersion: 1.06.2504030
osType: darwin-x64
time: 2025-10-17 18:35:41
```

![](assets/小程序开发-2025-10-17_18-28-18.png)
